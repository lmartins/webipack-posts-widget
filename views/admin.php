<?php
/**
 * Webipack Posts Widget: Widget Admin Form
 */
...
<div class="dpe-fp-widget">
	...
	<div class="section display">
		...
		<p class="cf">
			<label for="<?php echo $this->get_field_id( 'display_mode' ); ?>"><?php _e( 'Display Mode:', $this->get_widget_text_domain() ); ?></label>
			<select name="<?php echo $this->get_field_name( 'display_mode' ); ?>" id="<?php echo $this->get_field_id( 'display_mode' ); ?>">
				...
				foreach ($this->display_mode as $key => $value ) {
					echo '<option value="' . $key . '" id="' . $this->get_field_id( $key ) . '"', $display_mode == $key ? ' selected="selected"' : '', '>', $value , '</option>';
				}
				?>
			</select>
		</p>
